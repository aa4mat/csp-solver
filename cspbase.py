#  CISC 352 - W23
#  cspbase.py
#  desc: provides all the basic building blocks of a CSP, including a
#  Back Tracking based solver class.
#        Includes:
#            - Variables
#            - Constraints
#            - CSP
...
class Variable:
    ...
    """Class for defining CSP variables.  On initialization the
       ...
       (a) It has a current domain, implemented as a set of flags
           ...
           - you can obtain a list of values in the current domain, or count
             ...
             ### i.e. 2, 1 - {red, blue}, {red}
             ### -> use this for MRV as a heuristic
       ...
       (b) You can assign and unassign a value to the variable.
           ...
           search. # <- partial/complete assignments

           Assignments and current domain interact at the external interface
           level. Assignments do not affect the internal state of the current
           domain so as not to interact with value pruning and restoring during
           search.
           ...
           value.
           ### when WA = red, curr Domain(WA) = {red} only

           However, the internal state of the current domain
           flags are not changed so that pruning and unpruning can
           work independently of assignment and unassignment.

           ### if assigned -> return curr_assignment
                DO NOT change flags (already implemented)
    ...
    def __init__(self, name, domain=[]):
        ...
        self.dom = list(domain)        # Make a copy of passed domain - original
        self.curdom = [True] * len(domain)      # using list
        # truth assignments ordered as original domain of current Var object?
        # for bt_search
        self.assignedValue = None  # partial/ assgn
    ...
    """Internal methods starts here. DO NOT USE the below methods in your
     implementation."""
...
class Constraint:
    ...
    def check_tuple(self, t):
        ...
        return tuple(t) in self.sat_tuples
    ...
    def check_var_val(self, var, val):
        """Return `True` if we want to assign variable `var` with value
    ...
    '''Internal methods starts here. DO NOT USE the below methods in your implementation.'''
...
class CSP:
    ...
    def __init__(self, name, vars=[]):
        ...
        # dict mapping from var -> constraints with var in scope
        for v in vars:
            self.add_var(v)
    ...
    def get_all_nary_cons(self, size):
        '''return a list of all constraints of size n'''
        return [con for con in self.cons if len(con.scope) == size]
...
class BT:
    '''use a class to encapsulate things like statistics
       and bookeeping for pruning/unpruning variable domains
